...
Private Sub cdAttach_Click()
    mdApp.SetTableHistory
    
    Set Me.pcBall.Picture = fMain.pcGreen.Picture
    Set fRoll2.pcBall.Picture = fMain.pcRed.Picture
    fMain.cdTable1.BackColor = mdApp.LLGreen
...
Private Sub SetInitial()
    ...
    For ICounter = LBound(mdApp.IRollHT1) To UBound(mdApp.IRollHT1)
        ITotal = ITotal + mdApp.IRollHT1(ICounter)
        
        If mdApp.IRollHT1(ICounter) > 0 Then
            Me.lbRollCounter(ICounter).Caption = Format(mdApp.IRollHT1(ICounter), "#,##0")
        End If
    Next ICounter
    
    For ICounter = LBound(mdApp.IRollCHT1) To UBound(mdApp.IRollCHT1)
        Me.lbCounter(ICounter).Caption = " " & Format(mdApp.IRollCHT1(ICounter), "#,##0")
    Next ICounter
...
Private Sub SetDelete()
    If mdSecurity.EncryptText(Me.txPassword.Text, mdSecurity.SKey) = mdSecurity.SSecured Then
        mdApp.ClearRollHistory mdApp.IRollHT1, mdApp.IRollCHT1
